...
- Automated tests already cover behaviour, so use Playwright MCP for what they can't reach: layout, spacing, rendering, and visual appearance. Start the app, navigate every route the ticket touches, screenshot each, and confirm it renders correctly. Fix what you find, then move on.
