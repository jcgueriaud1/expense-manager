...
- **Source path:** `skills/figma-to-aura-theme/SKILL.md`
...
- **Locally modified:** no — copied unmodified, apart from this section.
- **Known upstream gap:** the skill references a `property-values.md` (the Aura named-colour,
  background and font tables) that does not exist at commit `3a9289c` — the repo ships only
  `SKILL.md`. Look those values up with `get_theme_css_properties theme=aura` instead. See
  finding F-060.
