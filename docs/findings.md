...
### F-051 — Receipt thumbnail on the line card only appeared after the report was saved
- Date: 2026-07-17
- Area: UX-spec
- Severity: Medium
- Task being attempted: Issue #89 — show the receipt image preview as soon as it
  is uploaded, not only after the report is saved.
- Expected vs actual: Expected the line card to show the image thumbnail the
  moment a receipt is attached (the line editor dialog already previewed the
  buffered bytes). Actual: the card rendered only a "📎 filename" text chip for a
  buffered (not-yet-persisted) receipt and switched to the real thumbnail only
  after the whole report was saved — because both card paths (manual line effect
  and travel generated-line row) rendered a `ReceiptPreview` only when
  `receiptId != null` (a persisted receipt streamed from the DB by id).
- Workaround used: added `ReportDetailView.receiptCardPreview(...)` — a persisted
  receipt still streams by id, but a buffered upload now previews straight from
  its in-memory bytes (`DownloadHandler.fromInputStream` over the `ReceiptUpload`
  in `pendingReceipts`/`pendingTravelReceipts`), falling back to the chip only
  when neither is in hand. Two ordering fixes were needed so the card sees the
  buffered bytes: `openEditor`/`openTravelLineReceipt` now buffer *before* the
  signal update, and `addLine` (whose `insertLast` builds the card + runs its
  effect before the buffer could be recorded) inserts the line receipt-free, puts
  the bytes, then `set`s the full dto so the effect re-runs with the buffer ready.
- Evidence: `ReportDetailView.java` (`receiptCardPreview`, `card` effect,
  `generatedLineRow`, `addLine`/`openEditor`/`openTravelLineReceipt`); new/updated
  browserless coverage in `ReportDetailViewUiTest`
  (`anUnsavedImageAttachmentPreviewsOnTheCardBeforeSave`,
  `uploadingAReceiptOnAnUnsavedReportPersistsItOnFirstSave`,
  `aTripListsItsGeneratedLinesAndAReceiptCanBeAttachedToOne`); verified live in
  Playwright (thumbnail + enlarge dialog on an unsaved report).
- Impact: the preview now matches user expectation and the in-dialog behaviour;
  no more "did my upload work?" ambiguity between attaching and saving.
- Note: the Vaadin 25.2 "Handle File Uploads" doc states the built-in
  `UploadHandler.inMemory()` callback runs on the UI thread (so direct component
  updates flush) — confirmed true here (the in-dialog preview and notification
  appear immediately with `@Push`). The bug was purely that the *card* deferred
  the thumbnail to a persisted id, not a push/threading problem.
- Owner / next step: resolved in this change.
