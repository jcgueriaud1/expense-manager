 ...
 *
 * <p>Everything except the receipt is read-only in the UI: the {@link #amount} is
 * recomputed from the trip inputs (the client never sends money) and the
 * {@link #comment} is the calculator's explanation. Only a receipt may be
 * attached — to any of the four kinds — which is why this carries the same
 * blob-free receipt summary fields as {@link ExpenseLineDto} (ADR-0021). The
 * {@link #vatRatePercent} lets the totals card split a VAT-bearing parking line
 * into Net/VAT live with the same {@code LineAmounts} maths.
 *
 ...
 * @param amount             gross amount, EUR scale 2 (recomputed, read-only)
...
public record GeneratedLineView(GeneratedLineKind kind, String expenseTypeName,
        ...
        Long receiptId, String receiptFilename, String receiptContentType,
        Long receiptSizeBytes) {

    /** A generated line with no receipt (preview / new-line construction). */
    public static GeneratedLineView of(GeneratedLineKind kind, String expenseTypeName,
            BigDecimal amount, BigDecimal vatRatePercent, String comment, Long lineId) {
        return new GeneratedLineView(kind, expenseTypeName, amount, vatRatePercent,
                comment, lineId, null, null, null, null);
    }
    ...
    public GeneratedLineView withReceipt(Long receiptId, String receiptFilename,
        ...
        return new GeneratedLineView(kind, expenseTypeName, amount, vatRatePercent,
                comment, lineId, receiptId, receiptFilename, receiptContentType,
                receiptSizeBytes);
    }
